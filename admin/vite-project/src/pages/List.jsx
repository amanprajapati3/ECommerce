      ...
      <div className="">
        ...
        <div className="flex flex-wrap sm:justify-start justify-center mt-4">
          {products.map((items, index) => (
            ...
            >
              <div>
                <img src={items.image} alt="" className="w-full sm:h-[230px]" />
                <p className="font-bold  my-2 text-center">{items.name}</p>
                <p className="font-medium text-center">Category : {items.category}</p>
                <p className="  text-center">Price : ${items.price}</p>
                <center>
                  <button onClick={()=> removeProduct(items._id )} className="text-white bg-red-700 hover:bg-red-600 active:bg-red-900 active:scale-95 cursor-pointer my-3 rounded-md px-8 py-1">Delete</button>
                </center>
              </div>
            </div>
          ))}
